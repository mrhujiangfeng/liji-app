...
本地 OCR 目前主要按照浙一报告版式优化，其他医院、模糊图片、压缩图片、特殊版式或小数点过淡时可能识别失败。遇到这种情况，建议先遮挡个人信息，再把报告图片交给豆包整理成文本，复制到粒迹默认展开的“文字录入”中。应用内可以一键复制下面这段提示词，也可点“放大录入”打开大文本框：

> 请识别这张检查报告，只输出可以直接复制的纯文本，不要解释。第一行写“检查日期：YYYY-MM-DD”，第二行写“检查机构：医院名称”；从第三行开始，每行依次输出：项目名称、检测结果、参考范围、单位，字段之间用制表符（Tab）分隔。不要遗漏项目，不要擅自修改数值；看不清的内容标记为“待核对”。
...
- 点“放大查看”会打开横向大画布。把手机横过来即可铺满查看曲线和表格；竖屏时也能横向滑动，并可在 30%—250% 之间缩放。
...
**为什么新版本安装后看起来仍是旧版？**  
确认下载的是 `Liji-CML-Tracker-0.5.2.apk`，并直接覆盖安装。若系统拒绝覆盖，通常是安装包签名不同；不要急着卸载，先从旧版导出完整备份。
